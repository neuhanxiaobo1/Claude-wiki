...
本页定义 ResearchWiki 的统一标签体系。新增标签前，先检查是否已有同义标签。领域专属标签应根据 `D/memory/project_profile.md` 逐步添加。
...
- 不确定是否应新增标签时，先写到 `D/inbox.md` 的“待整理标签”，并标注为“待确认”。
- 领域标签一旦确定，应在本方向 `D/memory/tag_taxonomy.md` 登记，不要写入公共词表或只散落在页面 frontmatter 中。公共词表仅登记跨方向的通用标签。
