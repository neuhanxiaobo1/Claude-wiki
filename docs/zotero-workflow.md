...
阅读时可使用已确认映射的 MinerU 正文；其 manifest 是解析缓存信息，与 D/raw/zotero_imports 中的候选 manifest 不是同一种记录。译文、正文、PDF、补充材料和视频分别标明实际覆盖。关键数据、图表和冲突按阅读规则回查原件；缺原件时保留已确认部分并限定未决判断。
